...
def calculate_score(event_data, used_event_list, entropy_weight):
    ...
    user_score_rank.to_csv('user_Issue_score_rank.csv', index=None)
...
if __name__ == '__main__':
    ...
    for name in event_data['user_login']:
        if name == 'I-am-a-robot' or name == 'mindspore_ci' or name == 'test-bot':
            index.append(count)
        if event_data['event_type'][count] == 'change_issue_state':
    ...
    used_event_list = ['create', 'assign_collaborator', 'setting_assignee', 'change_issue_state']
    ...
    # event_active.to_csv('Isue_event_count.csv', index=None)
    # print(event_active)
